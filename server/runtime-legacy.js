/**
 * server/runtime-legacy — 過渡期領域實作（由 api-proxy 機械遷移）
 * 新功能請寫到 server/routes/* 或 domain/*，不要再加長本檔。
 */
'use strict';
...
const { initStore, loadStore, saveStore, getStoreBackend } = require('../lib/enterprise-store');
const { initAuthStore, findUserByEmail, findUserById, createUser, updateUser, getAuthBackend } = require('../lib/auth-store');
const { initUserDataStore, getUserData, saveUserData, mergeUserData, ensureUserData, getUserDataBackend, defaultUserData } = require('../lib/user-data-store');
const { normalizeEmail, isValidEmail, clampText: clampAuthText, signToken, verifyToken, parseBearerToken, hashPassword, verifyPassword, sanitizeUser, getJwtConfig } = require('../lib/auth');
...
module.exports = {
    ...
    serveUploadFile,
    getReadiness,
    setCors, readBody, sendJson, handleRouteError, sendError, sendAccessResult,
    attachRequestLogging, parseQuery,
    checkRateLimit, checkAuthRateLimit, checkAiRateLimit,
    requireAuth, requireAiAuth, getAuthFromRequest, sanitizeChatBody,
    getStoreBackend, getAuthBackend, getUserDataBackend, getDatabaseStats,
    probeRagHealthDetail,
    initDb, ensureIndexes, initStore, initAuthStore, initUserDataStore,
    PORT, API_KEY, DEEPSEEK_URL, RAG_SERVICE_URL, serviceStartedAt,
    ragIndexEvents, ragBackgroundIndexJobs,
    AI_RATE_LIMIT_MAX, AI_RATE_LIMIT_WINDOW_MS, RAG_INDEX_TIMEOUT_MS,
    config
};
