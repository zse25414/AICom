...
(() => {
  ...
  var ENTERPRISE_SYNC_RETRY_MS = 5e3;
  var RAG_KB_LABELS = {
  ...
  var __luminaActions = /* @__PURE__ */ Object.create(null);
  ...
  function buildLuminaCallArgs(el, event) {
    ...
    if (el.dataset.luminaPassEvent !== void 0) {
      ...
      if (el.dataset.luminaArgFrom !== void 0) {
  ...
  async function runLuminaActionsFromElement(el, event) {
    ...
    if (el.dataset.luminaActions) {
      ...
      for (const item of chain) {
        const [name2, ...args] = item;
        await invokeLuminaAction(name2, event, args);
      }
      return;
    }
  ...
  function luminaAction(name, opts = {}) {
    ...
    if (opts.arg !== void 0) {
      const type = opts.type || (typeof opts.arg === "number" ? "number" : typeof opts.arg === "boolean" ? "boolean" : void 0);
  ...
  async function fetchRagKbList(groupCode) {
    ...
    try {
      ...
      const kb_ids = Array.isArray(data.kb_ids) && data.kb_ids.length ? data.kb_ids : items.map((i) => i.id).filter(Boolean);
  ...
  function rememberRagKbItems(items) {
    ...
    for (const item of items) {
      ...
      S.ragKbItemsById[item.id] = {
        id: item.id,
        displayName: item.displayName || prev?.displayName || RAG_KB_LABELS[item.id] || item.id,
  ...
  function extendFocusTimer(mins) {
    if (!S.focusSession?.endsAt) return;
    S.focusSession.endsAt += mins * 60 * 1e3;
    if (!S.focusTimerInterval) startFocusTimer(Math.ceil((S.focusSession.endsAt - Date.now()) / 6e4));
  ...
  function checkParentGoalComplete(task) {
    ...
    if (siblings.length > 1 && siblings.every((t) => t.completed)) {
      ...
      showToast(`\u{1F389} \u5927\u76EE\u6A19\u300C${name}\u300D\u7684\u6240\u6709\u6B65\u9A5F\u5DF2\u5B8C\u6210\uFF01`, "success");
  ...
  function scheduleEnterpriseSyncFlush() {
    ...
    S.enterpriseSyncFlushTimer = setTimeout(flushEnterpriseSyncQueue, ENTERPRISE_SYNC_RETRY_MS);
  }
    ...
    try {
      ...
      if (ragEl) {
        if (ragOk) {
          ...
          ragEl.textContent = bits.length ? `\u25CF \u5DF2\u9023\u7DDA (${bits.join(" \xB7 ")})` : "\u25CF \u5DF2\u9023\u7DDA";
          ...
          ragEl.title = apiReachable ? `API \u5DF2\u9023\u7DDA\uFF0CRAG \u672A\u5C31\u7DD2` + (errHint ? ` \u2014 ${errHint}` : "") : "\u8ACB\u57F7\u884C npm run dev";
        }
      }
    } catch (_) {
  ...
  async function renderServiceOpsPanel(apiStatus) {
    ...
    const jobs = ops?.backgroundIndexJobs != null ? ops.backgroundIndexJobs : apiStatus.backgroundIndexJobs;
    if (detailEl) {
      const lines = [
        `uptime: ${uptime != null ? uptime + "s" : "\u2014"}`,
        `store: ${details.store?.backend || "\u2014"}`,
        `auth: ${details.auth?.backend || "\u2014"}`,
        `rag: ${rag.ok ? "up" : "down"}` + (rag.latencyMs != null ? ` ${rag.latencyMs}ms` : "") + (rag.embedding ? ` \xB7 ${rag.embedding}` : "") + (rag.retrieval ? ` \xB7 ${rag.retrieval}` : "") + (rag.errorCode ? ` \xB7 ${rag.errorCode}` : ""),
    ...
    if (eventsEl) {
      ...
      if (!events.length) {
        ...
        eventsEl.innerHTML = events.slice(0, 8).map((ev) => {
          ...
          const code = ev.errorCode ? ` \xB7 ${escapeHtml(ev.errorCode)}` : "";
          ...
          const ms = ev.durationMs != null ? ` \xB7 ${ev.durationMs}ms` : "";
          const color = ev.outcome === "indexed" ? "text-emerald-400" : ev.outcome === "failed" ? "text-rose-400" : "text-amber-300";
          return `<div class="text-[10px] py-1 border-b border-slate-800/80 last:border-0">
                    ...
                    <span class="text-slate-500"> \xB7 ${title}</span>
  ...
  function applyEnterpriseTaskToCache(taskId, completed, serverTask) {
    ...
    if (serverTask) {
      ...
      task.completedAt = completed ? (/* @__PURE__ */ new Date()).toISOString() : null;
    }
    renderEnterpriseTasks();
    return true;
  }
  ...
  function updateRagQuerySummary() {
    ...
    if (emptyIds.length) {
      ...
      html += `<span class="coach-rag-query-empty-hint"> \xB7 \u7A7A\u5EAB\uFF1A${escapeHtml(emptyLabels.join("\u3001"))}</span>`;
    }
    el.innerHTML = html;
  ...
  function updateRagEmptyBanner(emptyIds) {
    ...
    textEl.innerHTML = `\u6240\u9078\u300C${escapeHtml(names.join("\u3001"))}\u300D\u5C1A\u7121\u6587\u4EF6\uFF0C\u6AA2\u7D22\u53EF\u80FD\u7121\u7D50\u679C\u3002\u8ACB\u81F3<button type="button" class="coach-rag-empty-link" data-lumina-action="openTeamKnowledgeTab">\u5718\u968A \u2192 \u77E5\u8B58\u5EAB</button>\u4E0A\u50B3\u3002`;
  }
  ...
  if (typeof window["getSourceRelevanceLevel"] !== "function") {
    window["getSourceRelevanceLevel"] = async function(...args) {
      await __loadChunk_coach();
      return window["getSourceRelevanceLevel"]?.(...args);
    };
  }
  if (typeof window["enrichCoachSource"] !== "function") {
    window["enrichCoachSource"] = async function(...args) {
      await __loadChunk_coach();
      return window["enrichCoachSource"]?.(...args);
    };
  }
  if (typeof window["closeCoachSourcePreview"] !== "function") {
    window["closeCoachSourcePreview"] = async function(...args) {
      await __loadChunk_coach();
      return window["closeCoachSourcePreview"]?.(...args);
    };
  }
  if (typeof window["openCoachSourcePreview"] !== "function") {
    window["openCoachSourcePreview"] = async function(...args) {
      await __loadChunk_coach();
      return window["openCoachSourcePreview"]?.(...args);
    };
  }
  if (typeof window["renderCoachSourceChips"] !== "function") {
    window["renderCoachSourceChips"] = async function(...args) {
      await __loadChunk_coach();
      return window["renderCoachSourceChips"]?.(...args);
    };
  }
  if (typeof window["renderCoachKbUsageBadge"] !== "function") {
    window["renderCoachKbUsageBadge"] = async function(...args) {
      await __loadChunk_coach();
      return window["renderCoachKbUsageBadge"]?.(...args);
    };
  }
  ...
  if (typeof window["resolveCoachKbSkipReason"] !== "function") {
    window["resolveCoachKbSkipReason"] = async function(...args) {
      await __loadChunk_coach();
      return window["resolveCoachKbSkipReason"]?.(...args);
    };
  }
  ...
  if (typeof window["resolveDocRagStatus"] !== "function") {
    window["resolveDocRagStatus"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["resolveDocRagStatus"]?.(...args);
    };
  }
  if (typeof window["applyLocalDocRagStatus"] !== "function") {
    window["applyLocalDocRagStatus"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["applyLocalDocRagStatus"]?.(...args);
    };
  }
  if (typeof window["ensureRagStatusPolling"] !== "function") {
    window["ensureRagStatusPolling"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["ensureRagStatusPolling"]?.(...args);
    };
  }
  if (typeof window["stopRagStatusPolling"] !== "function") {
    window["stopRagStatusPolling"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["stopRagStatusPolling"]?.(...args);
    };
  }
  if (typeof window["pollPendingDocumentRagStatus"] !== "function") {
    window["pollPendingDocumentRagStatus"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["pollPendingDocumentRagStatus"]?.(...args);
    };
  }
  if (typeof window["renderDocRagStatusBadge"] !== "function") {
    window["renderDocRagStatusBadge"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["renderDocRagStatusBadge"]?.(...args);
    };
  }
  if (typeof window["isNearEmptyDocContent"] !== "function") {
    window["isNearEmptyDocContent"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["isNearEmptyDocContent"]?.(...args);
    };
  }
  if (typeof window["retryDocumentRagIndex"] !== "function") {
    window["retryDocumentRagIndex"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["retryDocumentRagIndex"]?.(...args);
    };
  }
  if (typeof window["getDocCurrentVersion"] !== "function") {
    window["getDocCurrentVersion"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["getDocCurrentVersion"]?.(...args);
    };
  }
  if (typeof window["renderDocVersionBadge"] !== "function") {
    window["renderDocVersionBadge"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["renderDocVersionBadge"]?.(...args);
    };
  }
  if (typeof window["ensureDocUiState"] !== "function") {
    window["ensureDocUiState"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["ensureDocUiState"]?.(...args);
    };
  }
  if (typeof window["toggleDocVersionPanel"] !== "function") {
    window["toggleDocVersionPanel"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["toggleDocVersionPanel"]?.(...args);
    };
  }
  if (typeof window["toggleDocNewVersionForm"] !== "function") {
    window["toggleDocNewVersionForm"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["toggleDocNewVersionForm"]?.(...args);
    };
  }
  if (typeof window["restoreDocUiPanels"] !== "function") {
    window["restoreDocUiPanels"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["restoreDocUiPanels"]?.(...args);
    };
  }
  if (typeof window["loadDocumentVersions"] !== "function") {
    window["loadDocumentVersions"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["loadDocumentVersions"]?.(...args);
    };
  }
  if (typeof window["renderDocumentVersionList"] !== "function") {
    window["renderDocumentVersionList"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["renderDocumentVersionList"]?.(...args);
    };
  }
  if (typeof window["previewDocumentVersion"] !== "function") {
    window["previewDocumentVersion"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["previewDocumentVersion"]?.(...args);
    };
  }
  if (typeof window["renderDocumentVersionPreview"] !== "function") {
    window["renderDocumentVersionPreview"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["renderDocumentVersionPreview"]?.(...args);
    };
  }
  if (typeof window["closeDocumentVersionPreview"] !== "function") {
    window["closeDocumentVersionPreview"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["closeDocumentVersionPreview"]?.(...args);
    };
  }
  if (typeof window["publishDocumentVersion"] !== "function") {
    window["publishDocumentVersion"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["publishDocumentVersion"]?.(...args);
    };
  }
  ...
  if (typeof window["toggleCreateKbForm"] !== "function") {
    window["toggleCreateKbForm"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["toggleCreateKbForm"]?.(...args);
    };
  }
  if (typeof window["getFallbackKbItems"] !== "function") {
    window["getFallbackKbItems"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["getFallbackKbItems"]?.(...args);
    };
  }
  if (typeof window["populateTeamDocKbSelect"] !== "function") {
    window["populateTeamDocKbSelect"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["populateTeamDocKbSelect"]?.(...args);
    };
  }
  if (typeof window["renderTeamKnowledgeBases"] !== "function") {
    window["renderTeamKnowledgeBases"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["renderTeamKnowledgeBases"]?.(...args);
    };
  }
  if (typeof window["createTeamKnowledgeBase"] !== "function") {
    window["createTeamKnowledgeBase"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["createTeamKnowledgeBase"]?.(...args);
    };
  }
  if (typeof window["deleteTeamKnowledgeBase"] !== "function") {
    window["deleteTeamKnowledgeBase"] = async function(...args) {
      await __loadChunk_enterprise_docs();
      return window["deleteTeamKnowledgeBase"]?.(...args);
    };
  }
  ...
  registerLuminaAction("addTaskToList", addTaskToList);
  registerLuminaAction("advanceFocusStep", advanceFocusStep);
  registerLuminaAction("assignEnterpriseTask", assignEnterpriseTask);
  registerLuminaAction("clearAllTasks", clearAllTasks);
  registerLuminaAction("clearApiKey", clearApiKey);
  registerLuminaAction("closeMobileMore", closeMobileMore);
  registerLuminaAction("closeNotificationPanel", closeNotificationPanel);
  registerLuminaAction("closeTaskEdit", closeTaskEdit);
  registerLuminaAction("copyGroupCode", copyGroupCode);
  registerLuminaAction("createEnterpriseGroup", createEnterpriseGroup);
  registerLuminaAction("deleteTask", deleteTask);
  registerLuminaAction("dismissAuthAsGuest", dismissAuthAsGuest);
  registerLuminaAction("endFocusSession", endFocusSession);
  registerLuminaAction("exportData", exportData);
  registerLuminaAction("extendFocusTimer", extendFocusTimer);
  registerLuminaAction("focusQuickAdd", focusQuickAdd);
  registerLuminaAction("focusTodayTask", focusTodayTask);
  registerLuminaAction("handleLogin", handleLogin);
  registerLuminaAction("handleLogout", handleLogout);
  registerLuminaAction("handleRegister", handleRegister);
  registerLuminaAction("handleTeamNotificationClick", handleTeamNotificationClick);
  registerLuminaAction("initializeApp", initializeApp);
  registerLuminaAction("isSafeHttpUrl", isSafeHttpUrl);
  registerLuminaAction("joinEnterpriseGroup", joinEnterpriseGroup);
  registerLuminaAction("leaveEnterpriseGroup", leaveEnterpriseGroup);
  registerLuminaAction("markAllTeamNotificationsRead", markAllTeamNotificationsRead);
  registerLuminaAction("mergeTasksArrays", mergeTasksArrays);
  registerLuminaAction("navigateFromMobileMore", navigateFromMobileMore);
  registerLuminaAction("navigateFromMore", navigateFromMore);
  registerLuminaAction("nextOnboardingStep", nextOnboardingStep);
  registerLuminaAction("onRagKbCheckboxChange", onRagKbCheckboxChange);
  registerLuminaAction("openDecomposeTab", openDecomposeTab);
  registerLuminaAction("openTaskEdit", openTaskEdit);
  registerLuminaAction("openTeamKnowledgeTab", openTeamKnowledgeTab);
  registerLuminaAction("openUserMenu", openUserMenu);
  registerLuminaAction("optimizeSchedule", optimizeSchedule);
  registerLuminaAction("promptInstall", promptInstall);
  registerLuminaAction("quickAddTask", quickAddTask);
  registerLuminaAction("quickStartToday", quickStartToday);
  registerLuminaAction("recalculateInsights", recalculateInsights);
  registerLuminaAction("refreshEnterpriseData", refreshEnterpriseData);
  registerLuminaAction("refreshServiceStatus", refreshServiceStatus);
  registerLuminaAction("renderTaskList", renderTaskList);
  registerLuminaAction("resetAllData", resetAllData);
  registerLuminaAction("sanitizeHtml", sanitizeHtml);
  registerLuminaAction("saveSettings", saveSettings);
  registerLuminaAction("saveTaskEdit", saveTaskEdit);
  registerLuminaAction("setCategoryFilter", setCategoryFilter);
  registerLuminaAction("showAuthOverlay", showAuthOverlay);
  registerLuminaAction("showGuideTab", showGuideTab);
  registerLuminaAction("showSection", showSection);
  registerLuminaAction("skipOnboarding", skipOnboarding);
  registerLuminaAction("skipToNextTodayTask", skipToNextTodayTask);
  registerLuminaAction("splitTask", splitTask);
  registerLuminaAction("startTodayTask", startTodayTask);
  registerLuminaAction("switchAuthTab", switchAuthTab);
  registerLuminaAction("switchSchedulerTab", switchSchedulerTab);
  registerLuminaAction("switchTeamWorkspaceTab", switchTeamWorkspaceTab);
  registerLuminaAction("syncEnterpriseTaskToPersonal", syncEnterpriseTaskToPersonal);
  registerLuminaAction("testApiConnection", testApiConnection);
  registerLuminaAction("toggleDashStats", toggleDashStats);
  registerLuminaAction("toggleEnterpriseTask", toggleEnterpriseTask);
  registerLuminaAction("toggleMobileMore", toggleMobileMore);
  registerLuminaAction("toggleNavMore", toggleNavMore);
  registerLuminaAction("toggleNotificationPanel", toggleNotificationPanel);
  registerLuminaAction("toggleTaskComplete", toggleTaskComplete);
