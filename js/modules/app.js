/**
 * Lumina domain — merged from slices (auto-generated)
 * Edit files in js/modules/slices/ then npm run build:app
 */
import * as C from './core/constants.js';
import { S } from './core/store.js';

/* === Lumina slice: auth.js === */
...
function applyUserDataFromServer(data) {
    if (!data) return;
    if (Array.isArray(data.S.tasks)) {
        S.tasks = data.S.tasks;
    ...
    if (data.S.dailyHistory && typeof data.S.dailyHistory === 'object') {
        S.dailyHistory = data.S.dailyHistory;
        saveDailyHistory();
    }
    if (data.S.trackedFocusByDay && typeof data.S.trackedFocusByDay === 'object') {
        S.trackedFocusByDay = { ...S.trackedFocusByDay, ...data.S.trackedFocusByDay };
        saveTrackedFocus();
    }
    if (Array.isArray(data.S.weeklyScores) && data.S.weeklyScores.length === 7) {
        S.weeklyScores = data.S.weeklyScores;
...
async function loadUserDataFromServer() {
    ...
    try {
        ...
        const hasServer = Array.isArray(serverData?.S.tasks) && serverData.S.tasks.length > 0;

        if (hasServer && hasLocal) {
            applyUserDataFromServer({
                ...serverData,
                tasks: mergeTasksArrays(serverData.S.tasks, localTasks),
                dailyHistory: { ...(serverData.S.dailyHistory || {}), ...S.dailyHistory },
...
function importData(event) {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (data.S.tasks) {
                S.tasks = data.S.tasks.map((t, i) => sanitizeImportedTask(t, i)).filter(Boolean);
            }
            if (data.S.userProfile) {
                S.userProfile = { ...S.userProfile, ...sanitizeImportedProfile(data.S.userProfile) };
            }
            if (data.S.weeklyScores) {
                S.weeklyScores = data.S.weeklyScores.map(s => Math.min(100, Math.max(0, parseInt(s, 10) || 0)));
            }
            if (data.S.dailyHistory) S.dailyHistory = data.S.dailyHistory;
            if (data.S.trackedFocusByDay) {
                S.trackedFocusByDay = data.S.trackedFocusByDay;
                saveTrackedFocus();
            }
            migrateTasks();
...
function syncEnterpriseTaskToPersonal(enterpriseTaskId) {
    ...
    const et = (S.enterpriseGroupData.S.tasks || []).find(t => t.id === enterpriseTaskId);
    if (!et) return;
...
async function syncPersonalTaskCompletionToEnterprise(task) {
    ...
    const et = S.enterpriseGroupData?.S.tasks?.find(t => t.id === task.enterpriseTaskId);
...
function assignTasksToBlocks(pendingTasks, blocks) {
    ...
    for (const task of pool) {
        ...
        if (bestIdx >= 0) {
            slots[bestIdx].S.tasks.push(task);
    ...
    return { assigned: slots.filter(s => s.S.tasks.length > 0), remaining };
}
...
function optimizeSchedule(silent = false, force = false) {
    ...
    assigned.forEach(slot => {
        ...
        const tasksHTML = slot.S.tasks.map(t => `
        ...
        slotDiv.innerHTML = `
            ...
            <div class="flex-1 min-w-0">
                <div class="mb-2 flex items-center justify-between">
                    ...
                    <div class="text-xs px-2 py-px bg-emerald-500/10 text-emerald-300 rounded">${slot.S.tasks.length} 項任務</div>
                </div>
...
function renderEnterpriseTasks() {
    ...
    const myTasksEl = document.getElementById('team-my-S.tasks');
    ...
    const groupTasks = S.enterpriseGroupData.S.tasks || [];
    ...
    const statTasks = document.getElementById('team-stat-S.tasks');
...
function applyEnterpriseTaskToCache(taskId, completed, serverTask) {
    if (!S.enterpriseGroupData?.S.tasks) return false;
    const task = S.enterpriseGroupData.S.tasks.find(t => t.id === taskId);
...
function persistEnterpriseTaskToggle(taskId, completed) {
    const store = loadLocalEnterpriseStore();
    const group = store.groups[S.enterpriseSession.groupCode];
    const task = group?.S.tasks.find(t => t.id === taskId);
...
async function toggleEnterpriseTask(taskId, completed) {
    ...
    const snapshot = S.enterpriseGroupData?.S.tasks?.find(t => t.id === taskId);
...
function validateImportedData(data) {
    ...
    if (data.S.tasks !== undefined) {
        if (!Array.isArray(data.S.tasks)) throw new Error('S.tasks 必須是陣列');
        if (data.S.tasks.length > 2000) throw new Error('任務數量超過上限');
    }
    if (data.S.userProfile !== undefined && !isPlainObject(data.S.userProfile)) {
        throw new Error('S.userProfile 格式錯誤');
    }
    if (data.S.weeklyScores !== undefined) {
        if (!Array.isArray(data.S.weeklyScores) || data.S.weeklyScores.length !== 7) {
            throw new Error('S.weeklyScores 格式錯誤');
        }
    }
    if (data.S.dailyHistory !== undefined && !isPlainObject(data.S.dailyHistory)) {
...
function switchSchedulerTab(tab) {
    const tasksPanel = document.getElementById('scheduler-panel-S.tasks');
    const decomposePanel = document.getElementById('scheduler-panel-decompose');
    const tabTasks = document.getElementById('sched-tab-S.tasks');
...
export {
    ...
    dismissAuthAsGuest,
    checkAuthOnInit,
    verifyLocalManagerPin,
    loadDailyHistory,
    saveDailyHistory,
    loadTrackedFocus,
    saveTrackedFocus,
    getTrackedFocusMinutesForDate,
    recordFocusSessionMinutes,
    mergeTasksArrays,
    trimDailyHistory,
    snapshotDay,
    recordDailySnapshot,
    recalculateWeeklyScores,
    getFocusComparisonText,
    applyStreakReward,
    ...
    getRagKbLabel,
    syncDocumentToRag,
    reindexEnterpriseDocumentsToRag,
    ensureEnterpriseDocsInRag,
    ...
    getFuturePendingTasks,
    getTodayCompletedCount,
    getTodayFocusMinutes,
    getTodayCompletionRate,
    parseHour,
    scoreTaskForNextStep,
    rankTasksByNextStepScore,
    getNextRecommendedTask,
    resolveTodayFocusTask,
    ...
    getNextStepReason,
    renderPersonalTaskRow,
    getActiveParentGoals,
    checkParentGoalComplete,
    renderActiveGoalsPanel,
    renderTaskBadges,
    openTaskEdit,
    closeTaskEdit,
    saveTaskEdit,
    syncEnterpriseTaskToPersonal,
    syncEnterpriseCompletionToPersonal,
    enqueueEnterpriseSync,
    scheduleEnterpriseSyncFlush,
    flushEnterpriseSyncQueue,
    syncPersonalTaskCompletionToEnterprise,
    cacheEnterpriseGroupLocally,
    evaluateStreakOnComplete,
    focusQuickAdd,
    toggleDashStats,
    updateNextStepCard,
    ...
    loadLocalEnterpriseStore,
    saveLocalEnterpriseStore,
    normalizeEnterpriseCode,
    enterpriseFetch,
    enterpriseLocalCreate,
    enterpriseLocalJoin,
    enterpriseLocalGetGroup,
    toggleManagerPin,
    getMemberInitials,
    renderMemberChip,
    fetchApiReadiness,
    ...
    deleteTeamDocument,
    renderEnterpriseDocuments,
    renderEnterpriseTaskRow,
    assignEnterpriseTask,
    applyEnterpriseTaskToCache,
    persistEnterpriseTaskToggle,
    toggleEnterpriseTask,
    getEnterprisePollInterval,
    startEnterprisePolling,
    stopEnterprisePolling,
    ensureLocalGroupNotifications,
    pushLocalTeamNotification,
    getLocalTeamNotifications,
    getLocalReadNotificationStorageKey,
    loadLocallyReadNotificationIds,
    persistLocallyReadNotificationIds,
    rememberLocallyReadNotificationIds,
    applyLocalReadFlags,
    markLocalTeamNotificationsRead,
    getDefaultTeamNotificationPrefs,
    getTeamNotificationPrefs,
    saveTeamNotificationPrefs,
    onTeamDesktopNotifToggle,
    loadTeamNotificationPrefsForm,
    formatNotifTime,
    shouldAlertForNotification,
    ingestTeamNotificationsFromResponse,
    alertForNewTeamNotification,
    processIncomingTeamNotifications,
    refreshTeamNotifications,
    updateNotificationUI,
    renderNotificationPanel,
    ...
    handleTeamNotificationClick,
    pushCoachAgentMessage,
    getOpeningCoachMessage,
    ensureCoachSessionForTask,
    startStepTimerForCoach,
    coachBeginGuidedSession,
    coachPauseSession,
    coachAdvanceStepFromAgent,
    coachCompleteTaskFromAgent,
    buildOfflineAgentReply,
    inferAgentActionsFromUserMsg,
    isGenericCoachFallback,
    coachAgentRespondWithAI,
    findTaskForPlan,
    linkPlanToTask,
    syncFocusSessionWithPlan,
    parseJsonFromAI,
    ...
    parseBulletToField,
    ensureDocumentFields,
    renderEditableDocumentHtml,
    updateCoachDocField,
    toggleCoachChecklistItem,
    extractTaskNameFromMessage,
    inferTaskDocType,
    buildTaskResources,
    buildDocumentDraft,
    buildOfflineCoachPlan,
    coachPlanToMarkdown,
    renderCoachPlan,
    storeCoachPlan,
    copyCoachPlan,
    ...
    startCoachPlan,
    applyCoachStepsAsTasks,
    coachRespondWithAI,
    askCoachAboutNextTask,
    openCoachForTask,
    renderDecomposePlan,
    decomposeGoal,
    ...
    sendChatMessage,
    getCoachContext,
    buildCoachContextText,
    updateCoachContextBar,
    getCoachReadinessChecks,
    renderCoachReadinessBar,
    renderCoachQuickActions,
...
export function registerAppGlobals() {
    const fns = { initializeTailwind, clearSensitiveLocalData, hashPin, hashPassword, normalizeEmail, isValidEmail, getAuthBaseUrl, getAuthHeaders, authApiRequest, getAuthSession, isLoggedIn, needsAuthGate, persistAuthSession, applyAuthUserToProfile, clearAuthErrors, showAuthOverlay, hideAuthOverlay, switchAuthTab, updateAuthUI, buildUserDataPayload, applyUserDataFromServer, syncUserDataToServer, loadUserDataFromServer, finishAuth, handleRegister, handleLogin, handleLogout, openUserMenu, dismissAuthAsGuest, checkAuthOnInit, verifyLocalManagerPin, loadDailyHistory, saveDailyHistory, loadTrackedFocus, saveTrackedFocus, getTrackedFocusMinutesForDate, recordFocusSessionMinutes, mergeTasksArrays, trimDailyHistory, snapshotDay, recordDailySnapshot, recalculateWeeklyScores, getFocusComparisonText, applyStreakReward, evaluateStreakForDate, processDailyRollover, loadState, hasStoredApiKey, migrateApiSettings, migrateApiKeyStorage, getStoredApiKey, setStoredApiKey, getDeepSeekClientCredentials, isApiReady, updateApiStatusBadge, toggleApiModeFields, callDeepSeek, testApiConnection, loadSettingsForm, clearApiKey, saveSettings, exportData, importData, persistTasks, persistProfile, persistAnalytics, flushPersistState, saveState, getRagFilenameForDoc, getRagKbLabel, syncDocumentToRag, reindexEnterpriseDocumentsToRag, ensureEnterpriseDocsInRag, deleteDocumentFromRag, fetchRagKbIds, getRagServiceBase, getRagLlmCredentials, getRagQueryUrl, inferCategory, getCategoryLabel, getCategoryColor, resolveCategory, invalidateTodayStats, rebuildTaskIndex, getTaskById, rebuildTodayQueueMap, computeTodayStats, getTodayStats, getScoringContext, buildSyncedEnterpriseIdSet, migrateTasks, touchTask, addMinutes, getInitials, getFilteredTasks, getCategoryCounts, flushRefreshUI, refreshUI, refreshUIImmediate, setCategoryFilter, renderCategoryFilters, getCompletedCount, getTodayRelevantTasks, getTodayPendingTasks, getFuturePendingTasks, getTodayCompletedCount, getTodayFocusMinutes, getTodayCompletionRate, parseHour, scoreTaskForNextStep, rankTasksByNextStepScore, getNextRecommendedTask, resolveTodayFocusTask, getTodayQueuePosition, pulseNextStepCard, normalizeFocusSteps, buildQuickStartSteps, getStepsForTask, parseStepMinutes, getCoachTask, clearFocusTimer, tickFocusTimer, startFocusTimer, endFocusSession, extendFocusTimer, completeFocusTask, renderFocusSessionPanel, advanceFocusStep, focusTodayTask, startTodayTask, onTodayTaskCompleted, getNextStepReason, renderPersonalTaskRow, getActiveParentGoals, checkParentGoalComplete, renderActiveGoalsPanel, renderTaskBadges, openTaskEdit, closeTaskEdit, saveTaskEdit, syncEnterpriseTaskToPersonal, syncEnterpriseCompletionToPersonal, enqueueEnterpriseSync, scheduleEnterpriseSyncFlush, flushEnterpriseSyncQueue, syncPersonalTaskCompletionToEnterprise, cacheEnterpriseGroupLocally, evaluateStreakOnComplete, focusQuickAdd, toggleDashStats, updateNextStepCard, skipToNextTodayTask, quickAddTask, renderTaskList, toggleTaskComplete, splitTask, deleteTask, clearAllTasks, buildTimeBlocks, scoreTaskPriority, scoreTaskBlockFit, assignTasksToBlocks, optimizeSchedule, quickStartToday, ensurePdfJs, ensureXlsx, getEnterpriseBaseUrl, loadLocalEnterpriseStore, saveLocalEnterpriseStore, normalizeEnterpriseCode, enterpriseFetch, enterpriseLocalCreate, enterpriseLocalJoin, enterpriseLocalGetGroup, toggleManagerPin, getMemberInitials, renderMemberChip, fetchApiReadiness, formatReadinessHint, updateTeamSyncStatus, copyGroupCode, applyTeamInviteFromUrl, createEnterpriseGroup, joinEnterpriseGroup, leaveEnterpriseGroup, refreshEnterpriseData, renderEnterprisePage, renderEnterpriseTasks, toggleAddDocForm, switchDocFormType, handleDocFileSelect, extractTextFromPdf, extractTextFromExcel, saveTeamDocument, deleteTeamDocument, renderEnterpriseDocuments, renderEnterpriseTaskRow, assignEnterpriseTask, applyEnterpriseTaskToCache, persistEnterpriseTaskToggle, toggleEnterpriseTask, getEnterprisePollInterval, startEnterprisePolling, stopEnterprisePolling, ensureLocalGroupNotifications, pushLocalTeamNotification, getLocalTeamNotifications, getLocalReadNotificationStorageKey, loadLocallyReadNotificationIds, persistLocallyReadNotificationIds, rememberLocallyReadNotificationIds, applyLocalReadFlags, markLocalTeamNotificationsRead, getDefaultTeamNotificationPrefs, getTeamNotificationPrefs, saveTeamNotificationPrefs, onTeamDesktopNotifToggle, loadTeamNotificationPrefsForm, formatNotifTime, shouldAlertForNotification, ingestTeamNotificationsFromResponse, alertForNewTeamNotification, processIncomingTeamNotifications, refreshTeamNotifications, updateNotificationUI, renderNotificationPanel, toggleNotificationPanel, closeNotificationPanel, markTeamNotificationRead, markAllTeamNotificationsRead, handleTeamNotificationClick, pushCoachAgentMessage, getOpeningCoachMessage, ensureCoachSessionForTask, startStepTimerForCoach, coachBeginGuidedSession, coachPauseSession, coachAdvanceStepFromAgent, coachCompleteTaskFromAgent, buildOfflineAgentReply, inferAgentActionsFromUserMsg, isGenericCoachFallback, coachAgentRespondWithAI, findTaskForPlan, linkPlanToTask, syncFocusSessionWithPlan, parseJsonFromAI, parseCoachAgentResponse, decomposeGoalWithAI, normalizeCoachPlan, estimatePlanDuration, parseBulletToField, ensureDocumentFields, renderEditableDocumentHtml, updateCoachDocField, toggleCoachChecklistItem, extractTaskNameFromMessage, inferTaskDocType, buildTaskResources, buildDocumentDraft, buildOfflineCoachPlan, coachPlanToMarkdown, renderCoachPlan, storeCoachPlan, copyCoachPlan, downloadCoachDocument, startCoachPlan, applyCoachStepsAsTasks, coachRespondWithAI, askCoachAboutNextTask, openCoachForTask, renderDecomposePlan, decomposeGoal, generateSmartDecomposition, useExampleGoal, copyPlanToClipboard, addFirstStepToToday, addDecomposedToScheduler, getCoachWorkspace, formatCoachContent, renderCoachAgentThread, renderCoachEmptyState, renderCoachAgentView, coachStartFocusNow, refreshCoachView, askCoach, sendCoachAgentMessage, sendChatMessage, getCoachContext, buildCoachContextText, updateCoachContextBar, getCoachReadinessChecks, renderCoachReadinessBar, renderCoachQuickActions, openCoachForNextTask, generateManifestIcon, setupManifest, registerServiceWorker, updatePwaStatus, setupPwaInstall, promptInstall, setupOfflineDetection, toLocalISO, getTodayISO, getTomorrowISO, formatDateTW, getGreeting, escapeHtml, sanitizeHtml, isSafeHttpUrl, clampText, isPlainObject, sanitizeImportedTask, validateImportedData, sanitizeImportedProfile, sanitizeFaIcon, switchSchedulerTab, openDecomposeTab, clearOnboardHighlight, applyOnboardHighlight, renderOnboardingStep, startOnboarding, nextOnboardingStep, skipOnboarding, completeOnboarding, showGuideTab, closeNavMore, toggleNavMore, navigateFromMore, updateNavMoreState, toggleMobileMore, closeMobileMore, navigateFromMobileMore, showSection, setElText, setElHtml, setElStyle, updateDashboard, getEnergyLabel, syncCategoryFromEnergy, addTaskToList, getEnergyColor, getTimeDistribution, loadChartJs, refreshInsightsPage, updateInsightsCards, initCharts, recalculateInsights, triggerConfetti, showToast, resetAllData, refreshServiceStatus, setupKeyboardShortcuts, initializeApp };
