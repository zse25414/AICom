...
function sanitizeProfile(profile) {
    ...
    const allowed = [
        ...
        'streakThreshold', 'enableConfetti', 'enableExecProfile',
...
function sanitizeTasks(tasks) {
    ...
    return tasks.slice(0, MAX_TASKS).map(t => ({
        ...
        completedAt: t.completedAt ? String(t.completedAt).slice(0, 30) : null,
        ...
        sopDocId: t.sopDocId ? String(t.sopDocId).slice(0, 40) : null,
...
async function listAllUserData() {
    if (isUsingMongo()) {
        const docs = await getDb().collection('user_data').find({}).toArray();
        return docs.map(({ _id, ...rest }) => rest);
    }
    return Object.values(loadFromFile().users || {});
}
